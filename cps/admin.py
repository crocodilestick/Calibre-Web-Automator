    ...
    if db_change or not db_valid or not config.db_configured \
      or config.config_calibre_dir != to_save["config_calibre_dir"]:
        ...
        if db_change:
            ...
            ub.session.query(ub.PdfReaderState).delete()
            ub.session.query(ub.PdfReaderNote).delete()
...
def _delete_user(content):
    if ub.session.query(ub.User).filter(ub.User.role.op('&')(constants.ROLE_ADMIN) == constants.ROLE_ADMIN,
        ...
        if content.name != "Guest":
            ...
            ub.session.query(ub.PdfReaderState).filter(content.id == ub.PdfReaderState.user_id).delete()
            ub.session.query(ub.PdfReaderNote).filter(content.id == ub.PdfReaderNote.user_id).delete()
